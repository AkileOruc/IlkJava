...
public class C10_ArrayeBirElementEkleme {
    ...
    public static String[] arrayeElementEkleme(String[] eklenilecekArray, String eklenecekElement){
        String[] temp= new String[eklenilecekArray.length+1];  //sedece bir isim daha ekleyecegim icin +1 dedik
        for (int i = 0; i < eklenilecekArray.length ; i++) {
            temp[i]=eklenilecekArray[i];
        }
        temp[temp.length-1] =eklenecekElement;  //yeni temp array i nin son elementine Kemal i ekledik
        return temp;
    }
}